...
class HippoFormattedError
{
	 ...
	 * @return string
	 */
	private static function highlightSourceAtLocation(
		Source $source,
	 ...
	 * @return int
	 */
	private static function getColumnOffset(
		Source $source,
		SourceLocation $location
	) {
	...
	public static function createFromException(
		...
		if ($e instanceof ClientAware) {
			...
			if ($e->getPrevious()) {
				$formattedError["errorCode"] = $e->getPrevious()->getCode();
				$formattedError["errorMessage"] = $e
					->getPrevious()
					->getMessage();
			}
		} else {
		...
		if ($e instanceof Error) {
			$locations = Utils::map($e->getLocations(), static function (
				SourceLocation $loc
			) {
			...
			if (!empty($e->getExtensions())) {
				$formattedError["extensions"] =
					$e->getExtensions() + $formattedError["extensions"];
			}
		}

		if ($debug) {
			$formattedError = self::addDebugEntries(
				$formattedError,
				$e,
				$debug,
			);
	...
	public static function prepareFormatter(?callable $formatter = null, $debug)
	{
		$formatter =
			$formatter ?:
			static function ($e) {
				return HippoFormattedError::createFromException($e);
			};
		if ($debug) {
			$formatter = static function ($e) use ($formatter, $debug) {
				return HippoFormattedError::addDebugEntries(
					$formatter($e),
					$e,
					$debug,
				);
	...
	public static function toSafeTrace($error)
	{
		...
		return array_map(static function ($err) {
			$safeErr = array_intersect_key($err, [
				"file" => true,
				"line" => true,
			]);
			...
			if (isset($err["function"])) {
				$func = $err["function"];
				$args = !empty($err["args"])
					? array_map([self::class, "printVar"], $err["args"])
					: [];
	...
	public static function printVar($var)
	{
		...
		if (is_object($var)) {
			return "instance of " .
				get_class($var) .
				($var instanceof Countable ? "(" . count($var) . ")" : "");
		}
		if (is_array($var)) {
	...
	public static function create($error, array $locations = [])
	{
		...
		if (!empty($locations)) {
			$formatted["locations"] = array_map(static function ($loc) {
				return $loc->toArray();
			}, $locations);
		}
