...
class HippoGraphQL
{
	 ...
	 */
	public function queryAndReturnResult(
		string $query,
		?array $params = [],
		array $opts = []
	): ExecutionResult {
		...
		$errorFormatter = config("graphql.error_formatter", [
			static::class,
			"formatError",
		]);
		$errorsHandler = config("graphql.errors_handler", [
			static::class,
			"handleErrors",
		]);
	...
	protected function buildObjectTypeFromFields(
		array $fields,
		array $opts = []
	): ObjectType {
		$typeFields = [];
	...
	{
		if (
			isset($this->schemas[$name]) &&
			is_array($this->schemas[$name]) &&
			is_array($schema)
		) {
			$this->schemas[$name] = array_merge_recursive(
				$this->schemas[$name],
				$schema,
			);
		} else {
	...
	public function paginate(string $typeName, string $customName = null): Type
	{
		...
		if (!isset($this->typesInstances[$name])) {
			$paginationType = config(
				"graphql.pagination_type",
	 ...
	 * @return Type
	 */
	public function wrapType(
		string $typeName,
		string $customTypeName,
		string $wrapperTypeClass
	): Type {
	...
	public static function formatError(Error $e): array
	{
		$debug = config("app.debug")
			...
			: 0;
		$formatter = HippoFormattedError::prepareFormatter(null, $debug);
		$error = $formatter($e);
	...
	public static function handleErrors(
		...
		foreach ($errors as $error) {
			...
			if (
				...
				!($error instanceof Exception || $error instanceof PhpError)
			) {
				continue;
			}
	 ...
	 * @return string mixed
	 */
	public static function routeNameTransformer(
		string $name,
		string $schemaParameterPattern,
		string $queryRoute
	): string {
		...
		if (count($multiLevelPath) > 1) {
			if (Helpers::isLumen()) {
				array_walk($multiLevelPath, function (
					string &$multiName
				): void {
		...
		return $routeName ?:
			preg_replace(
				$schemaParameterPattern,
				"{" . (Helpers::isLumen() ? "$name:$name" : $name) . "}",
				$queryRoute,
			);
	}
